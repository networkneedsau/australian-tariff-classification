...
/**
 * Customs Tariff Act 1995 updater.
 */
export class CustomsTariffActUpdater extends BaseUpdater {
  readonly sourceId = 'customs_tariff_act';
  readonly sourceName = 'Customs Tariff Act 1995';
  readonly defaultCron = '0 3 1 * *';
  readonly targetTables = ['customs_tariff_act'];

  async fetch(): Promise<ActSection[]> {
    return scrapeActSections('Details/C2004A04560');
  }
  ...
  apply(db: Database.Database, data: ActSection[]): ApplyResult {
    ...
    const table = 'customs_tariff_act';
