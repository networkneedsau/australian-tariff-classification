        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field
              label="Duty Payable"
              value={
                dutyPayable !== null ? `$${dutyPayable.toFixed(2)}` : '\u2014'
              }
              highlight={dutyPayable === 0}
            />
          </div>
          <div className="grid grid-cols-2 gap-3 mt-3">
            <Field
              label="GST Base (Value + Duty)"
              value={
                dutyPayable !== null && cv > 0
                  ? `$${(cv + dutyPayable).toFixed(2)}`
                  : '\u2014'
              }
            />
            <Field
              label="GST Amount"
              value={gstAmount !== null ? `$${gstAmount.toFixed(2)}` : '\u2014'}
            />
          </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mt-3">
              <div className="flex justify-between items-center">
                ...
                <div className="text-right text-xs text-amber-600 space-y-0.5">
                  <div>Customs Value: ${cv.toFixed(2)}</div>
                  <div>
                    Duty ({entry.duty_rate}): ${dutyPayable!.toFixed(2)}
                  </div>
                  <div>
